...
namespace XMS.Core.Caching
{
	internal class LocalCacheManager
	{
		...
		private class InterlockedLinkedList
		{
			...
			/// 反向交换节点的值
			/// </summary>
			public void ReverseExchange(CachedItem item)
			{
		...
		private class NamedCache
		{
			...
			public Region GetRegion(string regionName)
			{
				...
				this.lock4Regions.EnterReadLock();
				try
				{
					if (this.regions.ContainsKey(regionName))
					{
				...
				{
					this.lock4Regions.ExitReadLock();
				}
		...
		private class Region
		{
			...
			private int asyncUpdateInterval = 10;
			...
			public void UpdateCacheItem(object state)
			{
				...
				try
				{
					...
					else
					{
						...
						// 重设当前缓存项的过期时间
						wrapper.Item.ExpiredTime = DateTime.Now.AddSeconds(Math.Min(this.asyncUpdateRetryInterval, this.asyncUpdateInterval) + this.asyncUpdateAdditionalLiveTime);
					}
				}
				catch (Exception err)
				{
		...
		private Dictionary<string, NamedCache> nameCaches = new Dictionary<string, NamedCache>(StringComparer.InvariantCultureIgnoreCase);

		private ReaderWriterLockSlim lock4Caches = new ReaderWriterLockSlim();


		private static CacheSetting GetCacheSetting(string cacheName)
		{
			CacheSettings settings = CacheSettings.Instance;

			if (settings.Caches.ContainsKey(cacheName))
			{
				return settings.Caches[cacheName];
			}
		...
		private NamedCache GetCache(string cacheName)
		{
			...
			if (!CacheSettings.Instance.Caches.ContainsKey(cacheName))
			{
			...
			this.lock4Caches.EnterReadLock();
			try
			{
				if (this.nameCaches.ContainsKey(cacheName))
				{
			...
			{
				this.lock4Caches.ExitReadLock();
			}

			if (cache != null)
			{
				if (cache.Dependency != null && cache.Dependency.HasChanged)
				{
					this.lock4Caches.EnterWriteLock();
					try
					{
						this.RemoveCacheInternal(cache);
					}
					finally
					{
						this.lock4Caches.ExitWriteLock();
		...
		public bool RemoveCache(string cacheName)
		{
			...
			try
			{
				return this.RemoveCacheInternal(cacheName);
			}
			finally
			{
				this.lock4Caches.ExitWriteLock();
		...
		private bool RemoveCacheInternal(string cacheName)
		{
			if (this.nameCaches.ContainsKey(cacheName))
			{
				NamedCache cache = this.nameCaches[cacheName];

				if (this.nameCaches.Remove(cacheName))
				{
					cache.Clear();
		...
		public void Clear()
		{
			...
			try
			{
				foreach (KeyValuePair<string, NamedCache> cache in this.nameCaches)
				{
					cache.Value.Clear();
				}

				this.nameCaches.Clear();
			}
			finally
			{
				this.lock4Caches.ExitWriteLock();
