...
namespace XMS.Core.Caching.Memcached
{
	...
	/// </summary>
	internal sealed class CustomMemcachedClientSection : ConfigurationSection, IMemcachedClientConfiguration
	{
		/// <summary>
		...
		{
			// 注意：默认使用 CustomKeyTransformer，以解决键过长的问题
			return this.KeyTransformer.CreateInstance() ?? new CustomKeyTransformer();
		...
		IServerPool IMemcachedClientConfiguration.CreatePool()
		{
			...
			{
				case MemcachedProtocol.Text: return new DefaultServerPool(this, new Enyim.Caching.Memcached.Protocol.Text.TextOperationFactory());
				case MemcachedProtocol.Binary: return new XMS.Core.Caching.Memcached.CustomBinaryPool(this);
			}
